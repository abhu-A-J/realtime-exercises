...
// listen for streams which opens for every request from browsers
server.on("stream", (stream, headers) => {
  ...
  if (path === "/msgs" && method === "GET") {
    ...
    // immediately reply with 200K and encoding
    stream.respond({
      ":status": 200,
    ...
    stream.on("close", () => {
      console.log("User disconnected from stream" + stream.id);
...
server.on("request", async (req, res) => {
  ...
  if (path !== "/msgs") {
    ...
    // not sending back information to user here
    res.end();
